...
class MCPServer {
  ...
  async init(): Promise<MCPServer> {
    ...
    try {
      ...
      logger.error(
        `服务器初始化失败: ${error instanceof Error ? error.message : error}`
      );
      throw error;
    }
  ...
  async start(): Promise<MCPServer> {
    ...
    try {
      // 获取端口配置
      const port = parseInt(process.env.PORT || '{{port}}') || 3000;
      ...
      logger.info(
        `{{projectName}} MCP 服务器已启动在端口 ${port} (Server-Sent Events)`
      );
    } catch (error) {
      logger.error(
        `服务器启动失败: ${error instanceof Error ? error.message : error}`
      );
      throw error;
    }
  ...
  async stop(): Promise<void> {
    if (this.server) {
      try {
        ...
        logger.error(
          `服务器停止时出错: ${error instanceof Error ? error.message : error}`
        );
      }
    }
  }
