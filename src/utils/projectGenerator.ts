...
export async function generateProjectWithProgress(
  ...
  try {
    ...
    reportProgress(
      GenerationStep.INSTALL_DEPENDENCIES,
      "项目依赖安装完成",
      true,
    );
    ...
    if (data.initGit) {
      ...
      try {
        ...
        const errorMsg =
          gitError instanceof Error ? gitError.message : "未知错误";
        reportProgress(
          GenerationStep.INIT_GIT,
          "Git 初始化失败，但项目创建成功",
          true,
          errorMsg,
        );
        console.warn(
          chalk.yellow("⚠️ Git 初始化失败，但项目创建成功："),
          errorMsg,
        );
      }
    }
...
async function copyDirectoryRecursively(
  ...
  for (const item of items) {
    ...
    // 这样做是为了避免 npm 自动将 .gitignore 重命名为 .npmignore 的行为
